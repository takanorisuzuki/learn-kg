...
> **注意：** `langchain-experimental` は実験的なパッケージのため、リリース間でAPIが予告なく変更される場合がある。`requirements.txt` でバージョンを固定すること（例：`langchain-experimental==0.0.65`）。
...
def load_documents(folder: str) -> list[Document]:
    """ローカルフォルダから.txtファイルを読み込む。信頼できるローカルパスのみに使用すること。"""
    docs = []
...
次のセッションでは、スキーマ注入とFew-shot Cypherの例を追加して、Text-to-Cypherの生成精度を本番品質に引き上げる。ここまではNeo4jとLangChainの公式ドキュメントに沿った標準的な使い方を辿ってきた。s06以降は、公式ドキュメントには書かれていない本番経験から生まれた設計パターンを扱う。技術的な難易度がここから上がる。
