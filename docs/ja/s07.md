...
s06のパイプラインに「まだどのエンジニアにも割り当てられていないバグを見せて」とクエリを投げると、結果は空になる。バグが全て割り当て済みだからではない。RAGはインデックスに存在しないものを見つけられないからだ。「BUG-007にはエンジニアがいない」というドキュメントは存在しない。リレーションの不在は、エンベッドして検索できるテキストを持たない。

これはプロンプトの問題でも、Few-shotの問題でもない。スキーマ注入は機能している。例も正しい。失敗の原因はアーキテクチャだ。

s03では、これをパターン5として紹介した。「否定クエリはRAGに原理的に不可能」。s06の動くパイプラインを持った今なら、その意味を実装レベルで体感できる。設定の問題ではなくアーキテクチャの問題として、正当なビジネスクエリが間違った結果を返す。
...
s04のNeo4jインスタンスに対してこれらのクエリを実行する。否定クエリはどんなデータでも動く。`ASSIGNED_TO` エッジが存在しないバグを見つけるだけだ。集計クエリとパス探索クエリは `Team` ノードと `BELONGS_TO` リレーションが必要になる。s04の「試してみる」でフルシードを投入していれば、すでに準備完了だ。
