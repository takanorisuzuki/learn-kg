...
def support_agent_with_kg(customer_id: str, question: str) -> str:
    ...
    # 注意：Ticket-[:REPORTED_BY]->Customer なので、Customerから逆方向に辿る
...
def record_agent_action(
    ...
    本番環境ではresultフィールドを書き込む前にサニタイズまたは切り詰めること。
    LLMが生成したコンテンツが含まれる可能性があるため。"""
    graph.query("""
...
def safe_write(query: str, params: dict, require_approval: bool = True) -> bool:
    ...
    if require_approval:
        ...
        # スタブ：実際のワークフローシステム（Slack承認、タスクキューなど）に置き換えること
        # input() は非インタラクティブ環境（Docker、CIなど）でブロックする
...
# サンプルデータを投入 — Customer・Ticketという新しいノード型を同じNeo4jインスタンスに追加する。
# s04で投入したEngineer・Bugノードとは独立しており、衝突なく共存する。
# MATCH (n) RETURN labels(n)[0], count(n) で全ノード型を一覧できる。
graph.query("""
...
# パターン1：READ
# Pattern 1: READ — Ticketから REPORTED_BY の方向でCustomerへ辿る
