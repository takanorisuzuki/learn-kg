...
s04のパイプラインは動く。そしてs05ではテキストからKGを自動構築した。しかし両者には共通の本番問題がある。ユーザーの質問は複雑だ。「先週の未解決チケットで、バックエンドチームに割り当てられているものを見せて」という質問に対し、LLMが存在しないプロパティ名でCypherを生成したり、リレーション型を発明したり、構文エラーを出したりする。

s05で `LLMGraphTransformer` が自動構築したグラフでは、スキーマが実行ごとに揺れるため、この問題がさらに深刻になる。LLMが毎回異なるノード型を推測しなければならない。

プロトタイプと本番の精度ギャップは現実のものだ。60%の質問に正確に答えられるパイプラインはビジネスでは使えない。Text-to-Cypherの生成精度を体系的に改善する方法が必要だ。

## 解決策

精度問題の大半は4つのテクニックで解決できる：

1. **スキーマ注入** — グラフ内の正確なノードラベル、プロパティ名、リレーション型をLLMに自動で伝える
2. **Few-shot Cypherの例** — 質問とCypherのペアを3〜5件見せてパターンを学習させる
3. **専用Cypher LLM** — Cypher生成に特化した強力なモデルを使う
4. **ハイブリッド検索ルーティング** — 構造的なクエリはKGへ、文脈的なクエリはベクトルRAGへ振り分ける

この4つを順番に適用すると、典型的なビジネスクエリでのText-to-Cypher精度が約60%から85%以上に向上する。
...
`PromptTemplate` は質問とCypherのペアを1件ずつラベル付きの文字列にフォーマットする。`FewShotPromptTemplate` は複数のフォーマット済み例を `prefix`（スキーマと指示）と `suffix`（実際の質問）の間にまとめる。結果として、LLMに参照例を複数見せた上で新しいCypherを生成させるプロンプトが1つできあがる。
...
# allow_dangerous_requests=True はCypher生成ステップへ到達する前に入力を検証する責任を
# 自分が負うことをLangChainに伝えるフラグ。LangChain ≥0.2 で必須。ないとチェーンが動かない。
# 入力バリデーションはs09のサンドイッチパターンの入力層で扱う。
# このセッションでは全てのテストクエリがハードコードされた文字列なので、外部入力はチェーンに届かない。
chain = GraphCypherQAChain.from_llm(
    ...
    verbose=True,
    allow_dangerous_requests=True,  # LangChain ≥0.2 で必須
...
sequenceDiagram
    ...
    G->>QL: {count: 2}
    QL->>U: "Aliceはcriticalバグを2件担当しています。"
```
...
```python
# ベースライン：Few-shotなし（s04から）— 1つのLLMがCypher生成と回答合成を兼任
baseline_chain = GraphCypherQAChain.from_llm(
    llm=cypher_llm, graph=graph, allow_dangerous_requests=True  # LangChain ≥0.2 で必須
)

# 改善版：スキーマ + Few-shot + 専用Cypher LLM
improved_chain = GraphCypherQAChain.from_llm(
    llm=qa_llm,          # 回答合成用の小さいモデル
    cypher_llm=cypher_llm,  # Cypher生成専用の大きいモデル
    graph=graph,
    cypher_prompt=cypher_prompt,
    validate_cypher=True,
    # ⚠️ allow_dangerous_requests=True は LangChain ≥0.2 で必須。本番環境ではユーザー入力を検証すること。
