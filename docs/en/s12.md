...
```bash
# On a VPS (2 vCPU, 8GB RAM is sufficient for Phase 2 — roughly $20–40/month on DigitalOcean, Hetzner, or Linode as of 2024)
...
```cypher
// Phase 3: grant read access at the graph level (Neo4j Enterprise RBAC)
GRANT MATCH (*) ON GRAPH kg TO read_only_user

// Note: Neo4j 5.x RBAC does not support row-level filtering via WHERE clauses.
// A DENY with WHERE (e.g., "only see your own team") is not valid syntax.
// For row-level access control, implement the filter in the application layer:
// add WHERE e.team = $user_team to each Cypher query before it reaches Neo4j.
```
...
In the next session, you will learn how to measure whether your KG project is actually working — and how to improve it systematically.
