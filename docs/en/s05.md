...
> **Note:** `langchain-experimental` is an experimental package. Its API can change without notice between releases. Pin your version in `requirements.txt` (e.g., `langchain-experimental==0.0.65`) to avoid unexpected breakage when updating.
...
In the next session, you will add schema injection and few-shot Cypher examples to bring Text-to-Cypher accuracy up to production quality. Until now, you have followed standard Neo4j and LangChain documentation patterns. From s06 onward, the sessions introduce design decisions built from production experience — patterns that are not in the official docs. The technical level increases from here.
