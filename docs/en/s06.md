...
Your pipeline from s04 works, and in s05 you built a KG automatically from unstructured text. But both face the same production problem: natural language queries are messy. Users ask "show me open tickets from last week assigned to people on the backend team." A naive LLM generates Cypher that references wrong property names, invents relationship types, or produces syntax errors.

With an auto-extracted graph from s05, schema inconsistency makes this worse — the LLM has to guess node types that `LLMGraphTransformer` may have labeled differently on each run.
...
Four techniques, applied in order, solve most accuracy problems:
...
3. **Dedicated Cypher LLM** — use a separate, stronger model for query generation
4. **Hybrid retrieval routing** — send structural queries to KG and contextual queries to vector RAG
...
`PromptTemplate` formats a single question/Cypher pair into a labeled string. `FewShotPromptTemplate` assembles multiple formatted examples between a `prefix` (the schema and instructions) and a `suffix` (the actual question to answer). The result is one prompt that shows the LLM several reference examples before asking it to generate a new Cypher query.
...
# allow_dangerous_requests=True tells LangChain that you accept responsibility
# for validating inputs before they reach the Cypher generation step. The flag is
# required in LangChain ≥0.2; without it the chain refuses to run. Validation is
# covered in the input layer of s09's sandwich pattern. For this session, all test
# queries are hardcoded strings — no external input reaches the chain.
...
sequenceDiagram
    ...
    G->>QL: {count: 2}
    QL->>U: "Alice has 2 critical open bugs."
```
...
```python
# Baseline: no few-shot (from s04) — single LLM handles both Cypher generation and answer synthesis
...
# Improved: with schema + few-shot + dedicated Cypher LLM
improved_chain = GraphCypherQAChain.from_llm(
    llm=qa_llm,         # smaller/faster model for answer synthesis
    cypher_llm=cypher_llm,  # larger model dedicated to Cypher generation
    graph=graph,
    cypher_prompt=cypher_prompt,
