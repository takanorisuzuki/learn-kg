...
You run the query "show me bugs not yet assigned to any engineer" against your s06 pipeline. The result comes back empty — not because all bugs are assigned, but because RAG cannot find what does not exist in the index. There is no document that says "BUG-007 has no engineer." The absence of a relationship leaves nothing to embed and retrieve.

This is not a prompt problem or a few-shot problem. The schema injection is working. The examples are correct. The failure is architectural.

In s03, you encountered this as Pattern 5 — negation queries listed as structurally impossible in RAG. Now, with a working pipeline from s06, you can feel the concrete impact: a legitimate business query returns wrong results because of architecture, not configuration.
...
Run these queries against your Neo4j instance from s04. The negation query works on any data — it finds bugs with no `ASSIGNED_TO` edge. The aggregation and path traversal queries require `Team` nodes and `BELONGS_TO` relationships; if you ran the full seed from s04's Try It section, these are already in place.

```cypher
// Negation: bugs not yet assigned (RAG cannot answer this — absence leaves nothing to embed)
MATCH (b:Bug)
WHERE NOT (b)-[:ASSIGNED_TO]->(:Engineer)
RETURN b.id, b.title, b.severity
