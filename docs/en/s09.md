...
# Enterprise KG Architecture Design
...
> **Warning:** The `validate_and_extract` function uses a blocklist approach for demonstration.
> For production use, replace with an allowlist that permits only expected Cypher patterns.
> The current implementation does not block CALL statements or other non-CRUD operations.
...
def validate_and_extract(raw_input: str) -> dict:
    ...
    # Note: this is a simplified illustrative example. In production, prefer
    # allowlist-based validation (only permit known query patterns) over
    # blocklist-based sanitization. Cypher injection via LLM is best mitigated
    # by using parameterized queries (shown in fetch_kg_context below) and
    # restricting the Neo4j user to read-only access.
...
def apply_rules(llm_output: str, user_role: str) -> str:
    ...
    # Role-based filtering (stub — add actual role restrictions here)
    # e.g., hide salary or PII fields for guest roles
