...
EVAL_DATASET = [
    {
        ...
        "expected_answer": "3",   # exact count — assumes test data seeded from s04 (3 critical bugs)
        "category": "count"
    },
    {
...
The repeating pattern from here:
1. Start with one use case (s12)
...
You now have everything to take KG from zero to production: the theory (s01-s03), the hands-on setup (s04-s05), the production techniques (s06-s07), the business context (s08), the architecture (s09), the agent integration (s10), the Traversal Contract (s11), the adoption strategy (s12), and the measurement cycle (s13).
