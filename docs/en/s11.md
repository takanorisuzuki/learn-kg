...
// Note: Neo4j 5.x RBAC does not support row-level filtering via WHERE.
// The DENY syntax above is pseudo-code for illustration only.
// For row-level access control (e.g., "only see your team's data"),
// implement the filter in the application layer — add WHERE e.team = $user_team
// to each Cypher query before it reaches Neo4j.
```
